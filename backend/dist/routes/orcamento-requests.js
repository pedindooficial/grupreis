...
const OrcamentoRequest_1 = __importDefault(require("../models/OrcamentoRequest"));
const Client_1 = __importDefault(require("../models/Client"));
const Budget_1 = __importDefault(require("../models/Budget"));
const Catalog_1 = __importDefault(require("../models/Catalog"));
const router = (0, express_1.Router)();
const orcamentoRequestSchema = zod_1.z.object({
    name: zod_1.z.string().min(1, "Nome obrigatório"),
    phone: zod_1.z.string().min(1, "Telefone obrigatório"),
    email: zod_1.z.string().email().optional().or(zod_1.z.literal("")),
    address: zod_1.z.string().optional(),
    latitude: zod_1.z.number().min(-90).max(90).optional(), // Latitude: -90 to 90
    longitude: zod_1.z.number().min(-180).max(180).optional(), // Longitude: -180 to 180
    ...
    sptDiagnostic: zod_1.z.string().optional(),
    services: zod_1.z.array(zod_1.z.object({
        serviceType: zod_1.z.string(),
        serviceTypeOther: zod_1.z.string().optional(),
        serviceId: zod_1.z.string().optional(), // Catalog service ID
        serviceName: zod_1.z.string().optional(), // Catalog service name
        diameter: zod_1.z.string().optional(),
        depth: zod_1.z.string().optional(),
        depthOther: zod_1.z.string().optional(),
        quantity: zod_1.z.string().optional(),
        quantityOther: zod_1.z.string().optional()
...
router.post("/", async (req, res) => {
    try {
        ...
        const lastRequest = await OrcamentoRequest_1.default.findOne()
            .sort({ seq: -1 })
            .lean();
        const seq = (lastRequest?.seq || 0) + 1;
        const request = await OrcamentoRequest_1.default.create({
            ...parsed.data,
            seq,
            status: "pendente",
...
router.get("/count/pending", async (req, res) => {
    try {
        await (0, db_1.connectDB)();
        const count = await OrcamentoRequest_1.default.countDocuments({ status: "pendente" });
        res.json({ data: { count } });
    }
    catch (error) {
...
router.get("/count/watch", async (req, res) => {
    try {
        ...
        // Send initial count
        const initialCount = await OrcamentoRequest_1.default.countDocuments({ status: "pendente" });
        ...
        const changeStream = OrcamentoRequest_1.default.watch([
            {
                $match: {
                    $or: [
        ...
        changeStream.on("change", async (change) => {
            try {
                // Recalculate count when there's a relevant change
                const count = await OrcamentoRequest_1.default.countDocuments({ status: "pendente" });
...
router.get("/", async (req, res) => {
    try {
        ...
        const requests = await OrcamentoRequest_1.default.find(query)
            .sort({ createdAt: -1 })
            .lean();
            ...
            try {
                ...
                if (change.operationType === "insert") {
                    ...
                    if (request) {
                        ...
                        res.write(`data: ${JSON.stringify({ type: "insert", request, requestId, operationType: change.operationType })}\n\n`);
                    }
                }
                    ...
                    if (change.fullDocument) {
                        ...
                        res.write(`data: ${JSON.stringify({ type: "update", request: change.fullDocument, requestId, operationType: change.operationType })}\n\n`);
                    }
                    else if (requestId) {
                        // Fetch the updated document
                        const request = await OrcamentoRequest_1.default.findById(requestId).lean();
                        if (request) {
                            res.write(`data: ${JSON.stringify({ type: "update", request, requestId, operationType: change.operationType })}\n\n`);
                        }
                    }
                }
                    ...
                    res.write(`data: ${JSON.stringify({ type: "delete", requestId, operationType: change.operationType })}\n\n`);
                }
                // Also send a refresh signal to fetch all requests
                if (isRelevant) {
                    const allRequests = await OrcamentoRequest_1.default.find()
                        .sort({ createdAt: -1 })
                        .lean();
                    res.write(`data: ${JSON.stringify({ type: "refresh", requests: allRequests })}\n\n`);
                }
            }
            catch (error) {
...
router.get("/:id", async (req, res) => {
    try {
        await (0, db_1.connectDB)();
        const request = await OrcamentoRequest_1.default.findById(req.params.id).lean();
        if (!request) {
...
router.patch("/:id", async (req, res) => {
    try {
        ...
        const updateSchema = zod_1.z.object({
            status: zod_1.z.enum(["pendente", "em_contato", "convertido", "descartado"]).optional(),
        ...
        const request = await OrcamentoRequest_1.default.findByIdAndUpdate(req.params.id, parsed.data, { new: true, runValidators: true }).lean();
        if (!request) {
            return res.status(404).json({ error: "Solicitação não encontrada" });
        }
        res.json({ data: request });
...
router.post("/:id/convert", async (req, res) => {
    try {
        ...
        const convertSchema = zod_1.z.object({
            createBudget: zod_1.z.boolean().optional().default(false),
            notes: zod_1.z.string().optional()
        });
        ...
        if (request.phone) {
            existingClient = await Client_1.default.findOne({ phone: request.phone });
        }
        if (!existingClient && request.email) {
            existingClient = await Client_1.default.findOne({ email: request.email });
        }
        // Also check by name and phone combination as a fallback
        if (!existingClient && request.phone && request.name) {
            existingClient = await Client_1.default.findOne({
                name: request.name,
                phone: request.phone
            });
        }
        ...
        if (!existingClient) {
            ...
            try {
                ...
                client = await Client_1.default.create(clientData);
                ...
                if (createError.code === 11000 || createError.message?.includes("duplicate key")) {
                    ...
                    if (isPersonTypeDocNumberDuplicate) {
                        ...
                        if (request.phone) {
                            ...
                            foundClient = await Client_1.default.findOne(queryByIndexAndContact);
                        }
                        ...
                        if (!foundClient && request.email) {
                            ...
                            foundClient = await Client_1.default.findOne(queryByIndexAndEmail);
                        }
                        ...
                        if (!foundClient && request.name) {
                            ...
                            foundClient = await Client_1.default.findOne(queryByIndexAndName);
                        }
                        // Last resort: just find any client with the duplicate index values
                        if (!foundClient) {
                            foundClient = await Client_1.default.findOne({
                                personType: "cpf",
                                $or: [
                                    { docNumber: null },
                                    { docNumber: { $exists: false } },
                    ...
                    if (!foundClient) {
                        // Strategy 1: By phone
                        if (request.phone) {
                            foundClient = await Client_1.default.findOne({ phone: request.phone });
                        }
                        // Strategy 2: By email
                        if (!foundClient && request.email) {
                            foundClient = await Client_1.default.findOne({ email: request.email });
                        }
                        ...
                        if (!foundClient && request.name) {
                            foundClient = await Client_1.default.findOne({ name: request.name });
                        }
                    }
                    if (foundClient) {
                        client = foundClient;
            ...
            if (hasNewAddress) {
                ...
                const addressExists = addresses.some((addr) => addr.address === request.address ||
            ...
            if (needsUpdate) {
                try {
                    await Client_1.default.findByIdAndUpdate(client._id, updateData, { new: true });
                    // Reload client to get updated data
                    client = await Client_1.default.findById(client._id);
        ...
        if (parsed.data.createBudget) {
            try {
                ...
                const mapSoilTypeToCatalog = (soilType) => {
                    if (!soilType)
                        return "outro";
                    const mapping = {
                ...
                const mapAccessToCatalog = (access) => {
                    if (!access)
                        return "livre";
                    const mapping = {
                        "facil": "livre",
                ...
                const budgetServices = await Promise.all(request.services.map(async (s) => {
                    ...
                    if (s.serviceId) {
                        try {
                            const catalogItem = await Catalog_1.default.findById(s.serviceId).lean();
                            if (catalogItem) {
                                ...
                                if (diametro && request.soilType && request.access && quantidade && profundidade) {
                                    ...
                                    if (!isNaN(diameterNum)) {
                                        const priceVariation = catalogItem.priceVariations?.find((pv) => pv.diameter === diameterNum &&
                ...
                budget = await Budget_1.default.create({
                    seq: budgetSeq,
        ...
        const updatedRequest = await OrcamentoRequest_1.default.findByIdAndUpdate(req.params.id, {
            status: "convertido",
            clientId: clientId,
        ...
        const clientData = await Client_1.default.findById(clientId).lean();
        res.json({
            data: {
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        await OrcamentoRequest_1.default.findByIdAndDelete(req.params.id);
