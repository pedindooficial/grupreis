...
router.get("/:id/pdf", async (req, res) => {
    try {
        await (0, db_1.connectDB)();
        const cashier = await Cashier_1.default.findById(req.params.id).lean();
        if (!cashier) {
        ...
        res.setHeader("Content-Disposition", `inline; filename="relatorio-caixa-${cashier._id.toString().slice(-6)}.pdf"`);
        doc.pipe(res);
        ...
        doc.font("Helvetica").text(formatDateTime(cashier.openedAt), 380, summaryY);
        summaryY += 15;
        if (cashier.closedAt) {
            doc.font("Helvetica-Bold").text("Fechado em:", 60, summaryY);
            doc.font("Helvetica").text(formatDateTime(cashier.closedAt), 140, summaryY);
        }
