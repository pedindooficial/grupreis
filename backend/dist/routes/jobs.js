...
const serviceSchema = zod_1.z.object({
    ...
    service: zod_1.z.string().min(1, "Selecione o serviço"),
    localType: zod_1.z.string().optional(),
    soilType: zod_1.z.string().optional(),
    access: zod_1.z.string().optional(),
    sptInfo: zod_1.z.string().optional(),
    sptFileName: zod_1.z.string().optional(),
...
router.get("/", async (_req, res) => {
    try {
        ...
        const jobs = await Job_1.default.aggregate([
            { $sort: { createdAt: -1 } },
            {
                $project: {
                    title: 1,
...
router.post("/", async (req, res) => {
    try {
        ...
        if (clientId) {
            ...
            if (client) {
                ...
                if (client.addresses && Array.isArray(client.addresses) && client.addresses.length > 0) {
                    ...
                    const matchingAddress = client.addresses.find((addr) => addr.address === site || addr.address === parsed.data.site);
        ...
        const processedServices = parsed.data.services.map((service) => {
        ...
        const totalFinalValue = processedServices.reduce((sum, s) => sum + (s.finalValue || s.value || 0), 0);
        const totalServiceValue = processedServices.reduce((sum, s) => sum + (s.value || 0), 0);
        const totalDiscountValue = processedServices.reduce((sum, s) => sum + (s.discountValue || 0), 0);
...
router.put("/:id", async (req, res) => {
    try {
        ...
        await (0, db_1.connectDB)();
        const existingJob = await Job_1.default.findById(req.params.id).lean();
        if (!existingJob) {
...
router.patch("/:id", async (req, res) => {
    try {
        ...
        if (updateData.value !== undefined || updateData.discountPercent !== undefined) {
            const existing = await Job_1.default.findById(req.params.id).lean();
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        const hasTransactions = await CashTransaction_1.default.exists({ jobId: job._id });
        if (hasTransactions) {
            return res.status(400).json({
...
const markReceivedSchema = zod_1.z.object({
    paymentMethod: zod_1.z.enum(["dinheiro", "pix", "transferencia", "cartao", "cheque", "outro"]).optional(),
    receipt: zod_1.z.string().optional(),
    receiptFileKey: zod_1.z.string().optional()
});
router.post("/:id/received", async (req, res) => {
    try {
        ...
        let openCashier = await Cashier_1.default.findOne({ status: "aberto" }).lean();
        if (!openCashier) {
            // Auto-open caixa with 0 balance
            const newCashier = await Cashier_1.default.create({
                status: "aberto",
                openingBalance: 0,
        ...
        if (existingTransaction) {
            ...
            const updated = await Job_1.default.findByIdAndUpdate(req.params.id, {
                received: true,
                receivedAt: new Date(),
        ...
        try {
            ...
            await session.withTransaction(async () => {
                ...
                const duplicateCheck = await CashTransaction_1.default.findOne({
                    jobId: job._id,
                    type: "entrada"
                })
                    .session(session)
                ...
                // Create transaction
                const [transactionDoc] = await CashTransaction_1.default.create([
                    {
                        type: "entrada",
                ...
                updated = await Job_1.default.findByIdAndUpdate(req.params.id, {
                    received: true,
                    receivedAt: new Date(),
                    receipt: parsed.data.receipt || undefined,
                    receiptFileKey: parsed.data.receiptFileKey || undefined
            ...
            const verifyTransaction = await CashTransaction_1.default.findOne({
                jobId: job._id,
                type: "entrada"
            }).lean();
...
const signatureSchema = zod_1.z.object({
    signature: zod_1.z.string().min(1, "Assinatura é obrigatória") // Base64 image
});
...
router.get("/:id/pdf", async (req, res) => {
    try {
        ...
        res.setHeader("Content-Disposition", `inline; filename="OS-${job.seq ? String(job.seq).padStart(6, "0") : "N/A"}.pdf"`);
        ...
        doc.text(`Gerado em ${new Date().toLocaleDateString("pt-BR")} às ${new Date().toLocaleTimeString("pt-BR")}`, pageWidth / 2 - 100, pageHeight - 30, { width: 200, align: "center" });
        // Finalize PDF
        doc.end();
    }
    catch (error) {
