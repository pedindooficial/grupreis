...
router.get("/financial", async (req, res) => {
    try {
        ...
        const prevTransactions = await CashTransaction_1.default.find({
            $or: [
...
router.get("/kpis", async (req, res) => {
    try {
        ...
        const completedJobsWithValue = jobs.filter((j) => j.status === "concluida" && (j.finalValue || j.value));
        const totalValue = completedJobsWithValue.reduce((sum, j) => sum + (j.finalValue || j.value || 0), 0);
